...
Original file is located at
    https://colab.research.google.com/drive/15defiQINGfPl13YrwgTlAjM_7d2w-Yea
"""
...
image_captions != -1]
spacy_eng = spacy.load("en_core_web_sm")8
vocab, embeddings = [], []
...
class Vocab:
  ...
  def numericalize(self, text):
    ...
    return [
    self.stoi[token] if token in self.stoi else self.stoi["<UNK>"] for
    token in tokenized_text]
...
num_layers=1):
    ...
    self.linear = nn.Linear(in_features=hidden_size, out_features=400000)
    self.softmax = nn.Softmax(dim=1)
  def forward(self, features, captions):
    embeddings = self.embed(captions)
    embeddings = torch.cat((features.unsqueeze(0), embeddings), dim=0)
    hiddens, _ = self.rnn(embeddings)
    packed = pack_padded_sequence(embeds, captions, batch_first=True)
...
class EncoderDecoder(nn.Module):
  ...
  num_layers=1): # vocab_size=vocab_size,
    ...
    self.decoder = DecoderLSTM(embed_size, hidden_size, num_layers) # vocab_size,
...
log_score_1, log_score_2, log_score_3, log_score_4 = [], [], [], []
for i in range(len(images_captions)):
  image_name = images_captions[i][0]
  caption = images_captions[i][1]
  test_img = transform(Image.open(img_dir + image_name)).unsqueeze(0)
  predicted_caption = " ".join(model.caption_image(test_img.to(device),
  dataset.vocab))
  score_1 = sentence_bleu(caption, predicted_caption, weights=(1, 0, 0, 0))
  score_2 = sentence_bleu(caption, predicted_caption, weights=(0.5, 0.5, 0,
  0))
  score_3 = sentence_bleu(caption, predicted_caption, weights=(0.33, 0.33,
  0.33, 0))
  score_4 = sentence_bleu(caption, predicted_caption, weights=(0.25, 0.25,
  0.25, 0.25))
  log_score_1.append(score_1)
  log_score_2.append(score_2)
  log_score_3.append(score_3)
  log_score_4.append(score_4)
print("#" * 25)
print("Cumulative Average Scores")
print("#" * 25)
print(f"BLEU-1: {sum(log_score_1) / len(log_score_1)}")
print(f"BLEU-2: {sum(log_score_2) / len(log_score_2)}")
print(f"BLEU-3: {sum(log_score_3) / len(log_score_3)}")
print(f"BLEU-4: {sum(log_score_4) / len(log_score_4)}")
print("#" * 25)
